...
# Server-IDs und Allowed-Role-IDs kommen NICHT aus .env — alles aus der DB.
...
def _parse_role_ids(raw) -> list[str]:
    """
    Parst Rollen-IDs aus DB-Feldern in eine bereinigte String-Liste.
    Unterstützt: None, "", "123,456", ["123","456"], [123,456]
    """
    if raw is None:
        return []
...
def _parse_added_users(raw) -> list[str]:
    """Parst tickets.added_users (JSONB) zu String-Liste."""
    if not raw:
        return []
    if isinstance(raw, list):
...
def _load_all_server_ids() -> list[str]:
    """
    Gibt alle Server-IDs zurück die in ticket_servers ODER application_servers
    konfiguriert sind. Das sind die Server auf die das Dashboard zugreifen kann.
    """
...
def _load_ticket_server_perms(server_id: str) -> dict:
    """ticket_servers.web_admin_role_ids für diesen Server."""
    key = f"tsp:{server_id}"
...
def _load_module_staff_map(server_id: str) -> dict[str, list[str]]:
    """
    {modul_name: [role_id, ...]} für alle Ticket-Module dieses Servers.
    Quelle: ticket_modules JOIN ticket_module_roles
    """
    key = f"msm:{server_id}"
    hit = _cget(_perm_cache, key, PERM_CACHE_TTL)
    if hit is not _MISS:
...
def _load_app_server_perms(server_id: str) -> dict:
    """application_servers.web_admin_role_ids + staff_role_ids für diesen Server."""
    key = f"asp:{server_id}"
    hit = _cget(_perm_cache, key, PERM_CACHE_TTL)
    if hit is not _MISS:
        return hit
...
# Session-Rollen
#
# session["user"]["server_roles"] = {
#     "1234567890": ["role_a", "role_b"],   ← Server 1
#     "9876543210": ["role_c"],              ← Server 2
# }
#
# Beim Berechtigungs-Check wird IMMER nur der Rollen-Set des angefragten
# Servers verwendet — nicht alle Rollen aller Server gemischt.
# ══════════════════════════════════════════════════════════════════════════════

def _get_user_roles_for_server(user: dict, server_id: str) -> set[str]:
    """
    Gibt die Discord-Rollen des Users auf einem bestimmten Server zurück.
    Liest aus session["user"]["server_roles"][server_id].
    """
...
# ══════════════════════════════════════════════════════════════════════════════
# Login-Ablauf: Welche Server darf der User sehen?
# ══════════════════════════════════════════════════════════════════════════════

def _build_server_roles(uid: str) -> dict[str, list[str]]:
    """
    Lädt für alle in der DB konfigurierten Server die Rollen des Users.
    Gibt ein Dict zurück: {server_id: [role_id, ...]}
    Nur Server auf denen der User Mitglied ist werden zurückgegeben.

    Beispiel:
      {
        "111": ["role_admin", "role_mod"],   ← User ist auf Server 111
        "333": ["role_staff"],               ← User ist auf Server 333
        # "222" fehlt → User ist nicht auf Server 222
      }
    """
    ...
    for sid in db_server_ids:
        ...
        if member and isinstance(member.get("roles"), list):
            ...
            log.debug(f"[auth] User {uid} nicht auf Server {sid} (oder Bot-Lookup fehlgeschlagen)")
...
def _user_has_dashboard_access(uid: str, server_roles: dict[str, list[str]]) -> bool:
    """
    Darf der User das Dashboard überhaupt nutzen?

    Ja wenn:
      - MBL
      - Hat auf irgendeinem Server eine Web-Admin/Staff/Modul-Staff Rolle
      - Hat ein Ticket erstellt (creator_id)
      - Wurde einem Ticket hinzugefügt (added_users)
      - Hat eine Bewerbung erstellt (creator_id)
    """
    if _is_mbl({"id": uid}):
        return True

    # Prüfe Rollen auf jedem Server
    for sid, roles in server_roles.items():
        ...
        if role_set & set(t_perms["web_admin_role_ids"]):
            log.info(f"[auth] {uid} hat Ticket-WebAdmin Zugriff auf Server {sid}")
            return True
        ...
        for role_list in staff_map.values():
            if role_set & set(role_list):
                log.info(f"[auth] {uid} hat Modul-Staff Zugriff auf Server {sid}")
                return True

        # Application Web-Admin oder Staff
        a_perms = _load_app_server_perms(sid)
        if role_set & set(a_perms["web_admin_role_ids"]):
            log.info(f"[auth] {uid} hat App-WebAdmin Zugriff auf Server {sid}")
            return True
        if role_set & set(a_perms["staff_role_ids"]):
            log.info(f"[auth] {uid} hat App-Staff Zugriff auf Server {sid}")
            return True

    # Fallback: eigene Tickets / Bewerbungen / added_users
    try:
        if sb("tickets").select("ticket_id").eq("creator_id", uid).limit(1).execute().data:
            log.info(f"[auth] {uid} hat Zugriff als Ticket-Ersteller")
            return True
    except Exception as e:
    ...
    try:
        if sb("applications").select("app_id").eq("creator_id", uid).limit(1).execute().data:
            log.info(f"[auth] {uid} hat Zugriff als Bewerbungs-Ersteller")
            return True
    except Exception as e:
    ...
    try:
        if sb("tickets").select("ticket_id").contains("added_users", [uid]).limit(1).execute().data:
            log.info(f"[auth] {uid} hat Zugriff als added_user")
            return True
    except Exception as e:
        log.error(f"[auth] added_users-check: {e}")

    log.warning(f"[auth] DENY: {uid} hat keinen Zugriff auf irgendeinen Server")
    return False
...
def can_see_ticket(user: dict, ticket: dict, server_id: str) -> bool:
    """
    Prüft Berechtigung für ein Ticket auf einem bestimmten Server.
    Verwendet NUR die Rollen des Users auf diesem Server.
    """
    if _is_mbl(user):
        return True
    ...
    # Web-Admin: alle Tickets
    t_perms       = _load_ticket_server_perms(server_id)
    web_admin_ids = set(t_perms["web_admin_role_ids"])
    if web_admin_ids:
        overlap = roles & web_admin_ids
        if overlap:
            log.debug(f"[perm] ticket#{tid}@{server_id}: ALLOW web_admin {overlap}")
            return True
        log.debug(f"[perm] ticket#{tid}@{server_id}: roles={roles} ∩ web_admin={web_admin_ids} = ∅")

    # Modul-Staff: nur Tickets dieses Moduls
    ...
    if module_staff:
        ...
        if overlap:
            log.debug(f"[perm] ticket#{tid}@{server_id}: ALLOW module_staff {overlap} (module={module_name})")
            return True
        log.debug(f"[perm] ticket#{tid}@{server_id}: roles={roles} ∩ staff[{module_name}]={module_staff} = ∅")

    # Ersteller
    if uid and str(ticket.get("creator_id", "")) == uid:
        log.debug(f"[perm] ticket#{tid}@{server_id}: ALLOW creator")
        return True

    # Hinzugefügter User
    if uid and uid in _parse_added_users(ticket.get("added_users")):
        log.debug(f"[perm] ticket#{tid}@{server_id}: ALLOW added_user")
        return True

    log.debug(f"[perm] ticket#{tid}@{server_id}: DENY user={uid} roles={roles}")
    return False


def can_see_application(user: dict, app: dict, server_id: str) -> bool:
    """
    Prüft Berechtigung für eine Bewerbung auf einem bestimmten Server.
    Verwendet NUR die Rollen des Users auf diesem Server.
    """
    if _is_mbl(user):
        return True
    ...
    if web_admin_ids:
        overlap = roles & web_admin_ids
        if overlap:
            log.debug(f"[perm] app#{aid}@{server_id}: ALLOW web_admin {overlap}")
            return True
        log.debug(f"[perm] app#{aid}@{server_id}: roles={roles} ∩ web_admin={web_admin_ids} = ∅")

    # Staff
    staff_ids = set(a_perms["staff_role_ids"])
    if staff_ids:
        overlap = roles & staff_ids
        if overlap:
            log.debug(f"[perm] app#{aid}@{server_id}: ALLOW staff {overlap}")
            return True
        log.debug(f"[perm] app#{aid}@{server_id}: roles={roles} ∩ staff={staff_ids} = ∅")

    # Ersteller
    if uid and str(app.get("creator_id", "")) == uid:
        log.debug(f"[perm] app#{aid}@{server_id}: ALLOW creator")
        return True

    log.debug(f"[perm] app#{aid}@{server_id}: DENY user={uid} roles={roles}")
    return False
...
def ensure_fresh_roles():
    """
    Refreshed server_roles in der Session wenn sie leer oder älter als
    ROLES_REFRESH Sekunden sind. Läuft vor jedem Request.
    """
    if "user" not in session:
        return
    ...
    # Member-Caches für alle bekannten Server leeren
    ...
    if roles_empty:
        log.info(f"[roles] ✅ server_roles für {uid} nachgeladen: {list(new_server_roles.keys())}")
    elif new_server_roles != old_server_roles:
        log.info(f"[roles] 🔄 server_roles für {uid} aktualisiert")
...
def login():
    ...
    params = urlencode({
        ...
        "response_type": "code",
        "scope":         "identify",   # Nur identify — Rollen kommen via Bot-Token
        "state":         state,
    })
...
def auth_callback():
    ...
    # ── Rollen auf allen konfigurierten Servern laden ─────────────────────────
    server_roles = _build_server_roles(uid)   # {server_id: [role_id, ...]}
    ...
    # ── Display-Name: vom ersten Server auf dem er gefunden wurde ─────────────
    ...
    session["user"] = {
        ...
        # server_roles: Map von server_id → [role_id, ...]
        # Nur Server auf denen der User Mitglied ist sind enthalten.
        "server_roles":     server_roles,
        "_roles_loaded_at": time.time(),
    }
    log.info(f"[auth] Login: {uid} ({display_name}) — "
             f"{len(server_roles)} Server: {list(server_roles.keys())}")
...
# ══════════════════════════════════════════════════════════════════════════════
# Seiten-Routen (HTML-Shell, sofort, keine Discord-Calls)
# ══════════════════════════════════════════════════════════════════════════════
...
def tickets():
    user      = session["user"]
    # Default: erster Server auf dem der User Zugriff hat (alphabetisch)
...
def _first_accessible_server(user: dict) -> str:
    """Gibt den ersten Server zurück auf dem der User Mitglied ist."""
    server_roles = user.get("server_roles") or {}
    if server_roles:
...
# ══════════════════════════════════════════════════════════════════════════════
# API: /api/guild — Server-Liste für Sidebar
# ══════════════════════════════════════════════════════════════════════════════
...
def api_guild():
    """
    Gibt nur die Server zurück auf denen der User tatsächlich Mitglied ist
    (d.h. server_roles enthält einen Eintrag für diesen Server).
    Getrennt nach ticket_servers und app_servers für die Sidebar-Navigation.
    """
    user         = session["user"]
    server_roles = user.get("server_roles") or {}
    ...
    def _enrich(rows):
        ...
        for row in rows:
            ...
            if not _is_mbl(user) and sid not in user_servers:
                continue  # User ist nicht auf diesem Server
            g = _cached_guild(sid)
            out.append({
                "server_id":  sid,
...
def api_tickets():
    ...
    # Prüfe ob User überhaupt auf diesem Server ist
    ...
    q = sb("tickets").select(
        "ticket_id,title,module,status,creator_id,creator_name,added_users,created_at"
...
# ══════════════════════════════════════════════════════════════════════════════
# API: /api/tickets/<id> — Ticket-Detail
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# API: /api/applications/<id> — Bewerbungs-Detail
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# API: /api/member/<uid> — Lazy Discord-Lookup
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# API: /api/debug/permissions — Diagnose
# ══════════════════════════════════════════════════════════════════════════════

@app.route("/api/debug/permissions")
@login_required
def api_debug_permissions():
    """Zeigt alle Berechtigungsdaten des aktuellen Users. Nur für MBL."""
    user = session["user"]
...
def api_refresh_roles():
    """Erzwingt einen sofortigen Rollen-Refresh aus dem Bot-API."""
    user = session["user"]
    uid  = user.get("id", "")

    # Alle Member-Caches leeren
    ...
    log.info(f"[refresh] Rollen für {uid} refreshed: {list(new_server_roles.keys())}")
    return jsonify({
        "refreshed": True,
        "servers":   {
